...
##################################
##QUERY ADDED TO MAINAPP#################
#################################################
	def QUERY(self):
		#set input boxes to variables
		self.Select = self.Entry1.get() 
           	self.From = self.Entry2.get()
           	self.Wherebool = self.CheckVar1.get()
           	self.Where1 = self.Entry3.get()
           	self.Where2 = self.Entry4.get()
		self.Logical1 = self.TCombobox1.get()

		print self.Select
		print self.From
		print self.Wherebool
		print self.Where1
		print self.Where2
		print self.Logical1
...
class QueryPage(tk.Frame):
	...
	# Run Query button
		...
		##########################################################
		#####################################################
		self.Button1 = tk.Button(command = lambda: controller.QUERY())
		###########################################################
		#########################################################
		######################################
